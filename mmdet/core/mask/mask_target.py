...
def mask_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg):
    ...
    num_pos = pos_proposals.size(0) # 10
    if num_pos > 0:
        proposals_np = pos_proposals.cpu().numpy() # 10, 4
        maxh, maxw = gt_masks.height, gt_masks.width # [1024, 1024]
        proposals_np[:, [0, 2]] = np.clip(proposals_np[:, [0, 2]], 0, maxw)
        proposals_np[:, [1, 3]] = np.clip(proposals_np[:, [1, 3]], 0, maxh)
        pos_assigned_gt_inds = pos_assigned_gt_inds.cpu().numpy() # gt
