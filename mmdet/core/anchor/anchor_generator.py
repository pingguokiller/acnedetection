...
class AnchorGenerator:
    ...
    def __init__(self,
        ...
        # base_sizes 就是根据配置文件中的 strides来的
    ...
    @property
    def num_base_anchors(self): # 这是一个函数，怪不得我找不到！！！！！
        """list[int]: total number of base anchors in a feature grid"""
        return self.num_base_priors

    @property
    def num_base_priors(self):  # 表示有多少个ratios, base_anchors:[3, 4]
    ...
    def gen_single_level_base_anchors(self,
        ...
        """
        w = base_size # [4, 8, 16, 32, 64] # 每一层的anchor的大小，实际上是具有不同的尺寸
        h = base_size
        if center is None:
        ...
        # ratios: [0.5, 1, 2]
        h_ratios = torch.sqrt(ratios) # [0.7, 1, 1.4]
        w_ratios = 1 / h_ratios # [1.4, 1, 0.7]

        # base_size * 将anchor比例 * scale[8]  #
        if self.scale_major: # True
            ws = (w * w_ratios[:, None] * scales[None, :]).view(-1) # [45, 32, 22]
            hs = (h * h_ratios[:, None] * scales[None, :]).view(-1) # [22, 32, 45]
        else:
        ...
        base_anchors = torch.stack(base_anchors, dim=-1) # [anchor_ratios(3), (x1,y1,x2,y2)(4)]
    ...
    def grid_anchors(self, featmap_sizes, device='cuda'):
        ...
        for i in range(self.num_levels):
            anchors = self.single_level_grid_anchors(
                ...
                self.strides[i],
                device=device)
            multi_level_anchors.append(anchors) #[[256*256*3, 128*128*3, ...], 4],
        return multi_level_anchors
    ...
    def single_level_grid_anchors(self,
        ...
        feat_h, feat_w = featmap_size # 每层feature_map的尺寸 [256, 128, ...]
        shift_x = torch.arange(0, feat_w, device=device) * stride[0] # 256 个 , [0, 4, ..., 1020]  # [4, 8, 16, 32, 64]
        shift_y = torch.arange(0, feat_h, device=device) * stride[1]

        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y)
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1)
        shifts = shifts.type_as(base_anchors) #  [256*256, (x1, y1, x2, y2)(4))]
        # first feat_w elements correspond to the first row of shifts
        # add A anchors (1, A, 4) to K shifts (K, 1, 4) to get
        # shifted anchors (K, A, 4), reshape to (K*A, 4)

        all_anchors = base_anchors[None, :, :] + shifts[:, None, :] # [3, 4] 的anchor 加上 所有的 shift anchor 进而得到所有的anchor
        all_anchors = all_anchors.view(-1, 4) # 256*256*3, 4
        # first A rows correspond to A anchors of (0, 0) in feature map,
        # then (0, 1), (0, 2), ...
        return all_anchors
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        multi_level_flags = []
        for i in range(self.num_levels):
            anchor_stride = self.strides[i] # [4, 4], ...
            feat_h, feat_w = featmap_sizes[i] # [256, 256], ...
            h, w = pad_shape[:2] # [1024, 1024]
            valid_feat_h = min(int(np.ceil(h / anchor_stride[1])), feat_h) # adapt to nopool #
            valid_feat_w = min(int(np.ceil(w / anchor_stride[0])), feat_w) # //2
            flags = self.single_level_valid_flags((feat_h, feat_w),
                                                  (valid_feat_h, valid_feat_w),
                                                  self.num_base_anchors[i], # [3, ...]
                                                  device=device)
            multi_level_flags.append(flags) # [256*256*3, ...] 全是1
    ...
    def single_level_valid_flags(self,
        ...
        valid_x = torch.zeros(feat_w, dtype=torch.bool, device=device) # //2
        valid_y = torch.zeros(feat_h, dtype=torch.bool, device=device) # //2
        valid_x[:valid_w] = 1
        valid_y[:valid_h] = 1
        valid_xx, valid_yy = self._meshgrid(valid_x, valid_y)
        valid = valid_xx & valid_yy
        valid = valid[:, None].expand(valid.size(0), num_base_anchors).contiguous().view(-1) # 256*256*3
        return valid
