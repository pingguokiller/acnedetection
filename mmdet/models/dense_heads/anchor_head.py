...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        if self.train_cfg is not None and not hasattr(self.train_cfg, 'iou_label'):
            self.train_cfg.iou_label = None
    ...
    # # RPNHead forward 的第二步; 根据不同的feature map forward_single(针对每个featuremap forward single)
    def forward_single(self, x, img_metas, **kwargs):
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        # feature_map_size = int(np.sqrt(labels.shape[1]/3))

        labels = labels.reshape(-1) # # 256*256*3*3
        label_weights = label_weights.reshape(-1) # [256*256*3*3, 128*128*3*3]
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(-1, self.cls_out_channels) # # 256*256*3*3

        # 用的是BCE 对这个二分类问题进行判断 label_weights: 采样正负样本标记为0
        ...
        # loss_cls = self.loss_bbox(cls_score, labels, label_weights, avg_factor=num_total_samples)
        ...
        # # misalignment
        # cls_score = cls_score.reshape(-1).reshape((img_num, feature_map_size, feature_map_size, 3))
        # bbox_pred = bbox_pred.reshape((img_num, feature_map_size, feature_map_size, 3, 4))
        #
        # label_weights = label_weights.reshape((img_num, feature_map_size, feature_map_size, 3))
        # label_weights_x = label_weights[:, :, :-1, :]
        # a = (cls_score[:, :, :-1, :] - cls_score[:, :, 1:, :]) #/((cls_score[:, :, :-1, :] + cls_score[:, :, 1:, :])/2)
        # b = (bbox_pred[:, :, :-1, :, 2] - bbox_pred[:, :, 1:, :, 2]) # /((bbox_pred[:, :, :-1, :, 2] + bbox_pred[:, :, 1:, :, 2])/2)
        # c = (bbox_pred[:, :, :-1, :, 0] - bbox_pred[:, :, 1:, :, 0]) # /((bbox_pred[:, :, :-1, :, 0] + bbox_pred[:, :, 1:, :, 0])/2)
        #
        # loss_width = torch.clamp(a*b*label_weights_x, min=0).sum()/num_total_samples
        # loss_x = torch.clamp(a*c*label_weights_x, min=0).sum()/num_total_samples
        #
        #
        # label_weights_y = label_weights[:, :-1, :, :]
        # e = (cls_score[:, :-1, :, :] - cls_score[:, 1:, :, :]) #/((cls_score[:, :-1, :, :] + cls_score[:, 1:, :, :])/2)
        # f = (bbox_pred[:, :-1, :, :, 2] - bbox_pred[:, 1:, :, :, 2]) # /((bbox_pred[:, :-1, :, :, 3] + bbox_pred[:, 1:, :, :, 3])/2)
        # g = (bbox_pred[:, :-1, :, :, 0] - bbox_pred[:, 1:, :, :, 0]) # /((bbox_pred[:, :-1, :, :, 1] + bbox_pred[:, 1:, :, :, 1])/2)
        #
        # loss_height = torch.clamp(e*f*label_weights_y, min=0).sum()/num_total_samples
        # loss_y = torch.clamp(e*g*label_weights_y, min=0).sum()/num_total_samples
        #print('loss_width, loss_x, loss_height, loss_y：', loss_width, loss_x, loss_height, loss_y)
        return loss_cls, loss_bbox  #+ 0.1*(loss_width + loss_x + loss_height + loss_y)
