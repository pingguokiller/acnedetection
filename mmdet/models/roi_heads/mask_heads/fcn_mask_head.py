...
class FCNMaskHead(BaseModule):
    ...
    def forward(self, x):
        for conv in self.convs:
            x = conv(x) # [N, 256, 14, 14] => [N, 256, 14, 14]
        if self.upsample is not None:
            x = self.upsample(x) # ConvTranspose2d [N, 256, 14, 14] => [N, 256, 28, 28]
            if self.upsample_method == 'deconv':
                x = self.relu(x)
        mask_pred = self.conv_logits(x) # [N, 256, 28, 28] => [N, 10, 28, 28]
        return mask_pred

    def get_targets(self, sampling_results, gt_masks, rcnn_train_cfg):
        pos_proposals = [res.pos_bboxes for res in sampling_results] # [N, 4]
        pos_assigned_gt_inds = [
            res.pos_assigned_gt_inds for res in sampling_results
        ] # [N, ...]
        mask_targets = mask_target(pos_proposals, pos_assigned_gt_inds, gt_masks, rcnn_train_cfg) # gt_masks
        return mask_targets
    ...
    def loss(self, mask_pred, mask_targets, labels):
        ...
        if mask_pred.size(0) == 0:
            ...
            if self.class_agnostic: # 类不可知
                loss_mask = self.loss_mask(mask_pred, mask_targets, torch.zeros_like(labels))
            else: # this way
                loss_mask = self.loss_mask(mask_pred, mask_targets, labels) # this way
