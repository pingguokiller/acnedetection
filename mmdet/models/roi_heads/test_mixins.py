...
class BBoxTestMixin:
    ...
    def simple_test_bboxes(self,
        ...
        # 先进行 ROI特征 提取, 然后 根据特征 进行分类与回归 # [bbox_num, [11类的score, 10类的bbox, 特征]]
        ...
        if bbox_pred is not None:
            ...
            if isinstance(bbox_pred, torch.Tensor):
                bbox_pred = bbox_pred.split(num_proposals_per_img, 0) # this way
            else:
        ...
        for i in range(len(proposals)):
            if rois[i].shape[0] == 0:
                ...
                if check_nms:
                    det_bbox, det_label, bbox_score_label_bfnms = self.bbox_head.get_bboxes(
                        rois[i],
                        cls_score[i],
                        bbox_pred[i],
                        img_shapes[i],
                        ...
                        cfg=rcnn_test_cfg,
                        check_nms=check_nms)
                    bbox_score_label_bfnms_list.append(bbox_score_label_bfnms)
                else:
        ...
        if check_nms:
            return det_bboxes, det_labels, bbox_score_label_bfnms_list
        else:
            return det_bboxes, det_labels
