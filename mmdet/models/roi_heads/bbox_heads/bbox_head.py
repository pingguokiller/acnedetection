...
class BBoxHead(BaseModule):
    ...
    def get_bboxes(self,
        ...
        else: # True # 先将11类进行softmax
            scores = F.softmax(cls_score, dim=-1) if cls_score is not None else None

        # bbox_pred would be None in some detector when with_reg is False,
        # e.g. Grid R-CNN.
        if bbox_pred is not None: # True 将10类 进行 bbox decode; roi 的第一列是imgid
        ...
        if cfg is None:
            ...
            if check_nms:
                det_bboxes, det_labels, bbox_score_label_bfnms = multiclass_nms(bboxes, scores, cfg.score_thr, cfg.nms, cfg.max_per_img, check_nms=check_nms)
                return det_bboxes, det_labels, bbox_score_label_bfnms
            else:
